...
**Template convergence unit SHIPPED (tpl-ios worker, supervisor-verified, aeae8df):** v3-local-templates migration + write-safe import (all three cases self-tested) + `pushLocalChanges` (templates→atomic adopt→sessions, abortive ordering, empty-list skips the endpoint — pre-deploy prod stayed safe) + TemplateEditorView (create via Workouts `+`, edit from briefing detail toolbar; name immutable post-create). Convergence loop proven end-to-end against a scratch local server (template pushed → id/version adopted → session restamped → linkage verified by curl). Server deployed to droplet same session.

**BUG TICKET (server, found by tpl-ios, NOT yet fixed):** on a sessions-sync error the server leaves its transaction open on the single pooled connection → every later write fails "cannot start a transaction within a transaction" until restart. Real clients don't hit the trigger path (malformed payload), but it's a poison-pill hazard — needs ROLLBACK-on-error in the sync handlers (check templates sync too, though it uses explicit ROLLBACK). Fix next server session.

**iOS template schema/import rulings (checkpoint, ratified):** GRDB migration `v3-local-templates` adds `templates.synced` (default 1, mirrors sessions v2). Locally-created = negative id; edited pulled template KEEPS its positive server id + synced=0 (session linkage + replace-by-id reconcile). Import: `DELETE WHERE synced=1`, insert server rows EXCEPT ids matching surviving synced=0 rows (in-flight edit preserved until push+pull adopts authoritative copy). ADOPT after push = match-by-name → restamp unsynced sessions to server id+version → delete local rows, ALL IN ONE TRANSACTION; **ordering abortive: template-push failure blocks the sessions push that refresh** (negative template_id structurally cannot reach the server). Accepted window: adopted-but-not-yet-pulled template briefly unrenderable locally; next pull heals. Self-test must prove all three import cases. Briefing TO-BEAT targets re-anchored at the incumbent's own coordinates (last-session anchoring ballooned after light days — "≥24 at 60kg" incident).
